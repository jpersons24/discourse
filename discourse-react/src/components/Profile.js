...
const Profile = ({ currentUser, matchedUser, setMatchedUser,setHasActiveChat, hasActiveChat, setCurrentUser, allUsers }) => {
  ...
  const matchMaker = () => {
    ...
    fetch(`http://localhost:3001/users/${currentUser.user.id}`, {
      ...
      body: JSON.stringify({ ...currentUser.user, differences: finalDifferences, interests: finalInterests}),
      })
      ...
      body: JSON.stringify({ ...collectedArray[0], differences: finalDifferences, interests: finalInterests}),
      })
      .then(response => response.json())
      .then((data) => {
        // setMatchedUser(data);
  ...
  return (
    <div>
      ...
      <h2>Your match has these differences {currentUser.user.differences} {currentUser.user.interests}</h2>
